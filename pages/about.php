<section id="team" class="team-area pb-130" style="background-color:#efe9c2;">
        <div class="container mt-3 py-5">
            <div class="row justify-content-center">
                <div class="col-lg-6">
                    <div class="section-title text-center pb-20 fnt">
                        <h5 class="sub-title mb-15 ">Meet CS MEMBER</h5>
                        <h2 class="title ">Our Expert Team</h2>
                    </div> <!-- section title -->
                </div>
            </div> <!-- row -->
            <div class="row">
                <div class="col-lg-4 col-md-6 col-sm-6">
                    <div class="single-team text-center mt-30">
                        <div class="team-image">
                            <img src="image/team/yay.jpg" alt="Team">
                        </div>
                        <div class="team-content">
                            <h4 class="">Ahmad Nur Hidaya</h4>
                            <span class="sub-title">2010130002</span>
                            <ul class="social mt-4">
                                <li><a href="https://www.instagram.com/ya2_inie/"><i class="fab fa-instagram"></i></a></li>
                                <li><a href="#"><i class="lni-facebook-filled"></i></a></li>
                                <li><a href="#"><i class="lni-twitter-original"></i></a></li>
                            </ul>
                        </div>
                    </div> <!-- single team -->
                </div>
                <div class="col-lg-4 col-md-6 col-sm-6">
                    <div class="single-team text-center mt-30">
                        <div class="team-image">
                            <img src="image/team/Refido.jpeg" alt="Team">
                        </div>
                        <div class="team-content">
                            <h4 class="">Refido Arjunal Akmal</h4>
                            <span class="sub-title">2010130013</span>
                            <ul class="social mt-4">
                                <li><a href="https://www.instagram.com/refidoarjunalakmal/"><i class="fab fa-instagram"></i></a></li>
                                <li><a href="#"><i class="fab fa-facebook-square"></i></a></li>
                                <li><a href="#"><i class="fab fa-twitter-square"></i></a></li>
                            </ul>
                        </div>
                    </div> <!-- single team -->
                </div>
                <div class="col-lg-4 col-md-6 col-sm-6 "id="ryo">
                    <div class="single-team text-center mt-30">
                        <div class="team-image">
                            <img src="image/team/Ryo.jpg" alt="Team">
                        </div>
                        <div class="team-content">
                            <h4 class="">Ryo Resha Herdiansyah</h4>
                            <span class="sub-title">2010130014</span>
                            <ul class="social mt-4">
                                <li><a href="https://www.instagram.com/ryorsh_22/"><i class="lni-instagram-original"></i></a></li>
                                <li><a href="#"><i class="lni-facebook-filled"></i></a></li>
                                <li><a href="#"><i class="lni-twitter-original"></i></a></li>
                            </ul>
                        </div>
                    </div> <!-- single team -->
                </div>
            </div> <!-- row -->
        </div> <!-- container -->
    </section>